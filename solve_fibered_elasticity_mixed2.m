...
function [geometry, msh, space_u, u] = ...
              solve_fibered_elasticity_mixed2 (problem_data, method_data)
...
space_p = sp_bspline(geometry.nurbs.knots,[0, 0],msh); 
ndof_lam = nsub(1)*nsub(2)*3;
ndof_p = nsub(1)*nsub(2);

% Assemble the matrices
A = op_eu_ev_tp(space_u, space_u, msh, mu_lame);
...
E = op_div_v_q_tp(space_u, space_p, msh, ndof_p)';
G = op_p_phi_tp(space_p, space_p, msh, ndof_p);
H = op_lam_theta_I_phi_I_tp(space_p, space_p, msh, lambda_lame, ndof_p);
...
K = [A, sparse(nu, nl), sparse(nu, np), B, E;...
    sparse(nl, nu), D, sparse(nl, np), -C, sparse(nl, np);...
    sparse(np, nu), sparse(np, nl), H, sparse(np, nl), -G;...
    B', -C', sparse(nl, np), sparse(nl, nl), sparse(nl, np);...
    E', sparse(np, nl), -G', sparse(np,nl), sparse(np, np)];
% K = [A, B, sparse(nu, nl);
%      B', sparse(nl, nl), -C;
%     sparse(nl, nu), -C', D];
